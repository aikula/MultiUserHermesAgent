...
> **Отступление в реализации:** tracking делает webapp в `app/quota.py` при каждом вызове (из `chat.completions.usage.total_tokens`), а НЕ парсер логов gateway — потому что gateway не знает про наших юзеров (его `session_id` — это его внутренние сессии, не маппятся на uid через `auth.json`).
...
### Фаза 4 — Per-user memory ✅ Done (2026-06-02)
- ✅ Webapp пишет в `users/<uid>/memory.md` после каждых N (default 20) сообщений (через summarizer LLM)
- ✅ При запросе webapp подмешивает `memory.md` в system prompt (после SOUL.md)
- ✅ Telegram-релей делает то же самое
- ✅ Реализация: `webapp/app/summarizer.py` — per-user `asyncio.Lock`, инкрементальное обновление, structured Markdown
- **Done when:** агент помнит факты о юзере между сессиями ✓

### Фаза 5 — Квоты ✅ Done (2026-06-02)
- ✅ Webapp `app/quota.py`: `record(uid, channel, tokens)` — декрементирует `users.quota_remaining`, инкрементирует `quota_used`, обновляет `/root/.hermes-app/quotas/<uid>/<YYYY-MM-DD>.json` атомарно
- ✅ `get_usage(uid)` — breakdown: used/remaining/pct, today/month, by-channel
- ✅ `/api/usage` endpoint + `/profile` с progress bar
- ✅ Alert: WARNING-лог при pct ≥ `ALERT_THRESHOLD_PCT` (80%), throttle на +5%; опционально TG admin
- ✅ ENV: `ALERT_THRESHOLD_PCT=80`, `TELEGRAM_ADMIN_CHAT_ID=`
- **Done when:** вижу расход по юзерам в /api/usage и /profile, alert при приближении к лимиту ✓
- **Отступление:** tracking делает webapp из `chat.completions.usage.total_tokens` (а не парсер логов gateway — gateway не знает про наших юзеров)

### Фаза 6 — Бэкапы и операционка ✅ Done (2026-06-02)
- ✅ Скрипт `cron/backup.sh`: ежедневно в 03:30 (crontab установлен)
  - `users.db` → `backups/users-YYYY-MM-DD.db` через Python `sqlite3.backup()` (атомарно для WAL)
  - `users/<uid>/` → `backups/users-YYYY-MM-DD/`
  - Ротация: 7 последних дней (find -mtime +7 -delete)
  - Логи: `/root/.hermes-app/logs/backup-YYYY-MM-DD.log`
- ✅ Симлинки `/root/.hermes-app/cron/{backup.sh,README.md}` → `/root/Agents/Hermes/cron/`
- ✅ README с инструкциями восстановления
- **Done when:** проверено вручную, бэкап users.db+users/ работает, ротация удаляет >7 дней
